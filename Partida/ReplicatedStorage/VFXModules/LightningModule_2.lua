...
local function DiscretePulse(input, s, k, f, t, min, max) --input should be between 0 and 1. See https://www.desmos.com/calculator/hg5h4fpfim for demonstration.
	return math.clamp( (k)/(2*f) - abs( (input - t*s + 0.5*(k)) / (f) ), min, max )
end
...
function LightningBolt.new(Attachment0, Attachment1, PartCount)
	...
	local parent = game.Workspace.VFX --workspace.CurrentCamera
			...
			if timePassed < (PulseLength + 1) / PulseSpeed then
				...
				for i = 1, PartsN do
					...
					elseif Opacity > contractf - 1/(PartsN*FadeLength) then
						...
						BPart.Size = Vector3.new((1 - abs(interp))*(NextPoint - PrevPoint).Magnitude, thickness*thicknessNoise*Opacity, thickness*thicknessNoise*Opacity)
						BPart.CFrame = CFrame.lookAt(PrevPoint + (NextPoint - PrevPoint)*(math.max(0, interp) + 0.5*(1 - abs(interp))), NextPoint)*xInverse
						BPart.Transparency = 1 - Opacity
					else
