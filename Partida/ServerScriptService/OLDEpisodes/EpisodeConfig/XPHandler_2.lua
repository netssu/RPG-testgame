...
XPHandler.RedeemTiers = function( Player : Player )
	...
	for Index = 1 , clamp( XPHandler.GetCurrentTier( EpisodePass[stringd].Value :: number ) - 1 , 0 , MaxTier ) do
		for freeopremium , titlenamount in TierInfo[Index] do
			...
			if RewardsClaimed:FindFirstChild(wannaname) then
				continue
			end
			--
			local Title = titlenamount.Title
			--print( Title and Title:find('SHINY') )
			...
			if justavalue then
				...
				for indexyuh = 1 , titlenamount.Amount do
					_G.createTower( Player.OwnedTowers , Title , nil , { Shiny = Title:find('SHINY') and true or false } )
				end
			end
			--
...
if game:GetService('RunService'):IsServer() then
	local RequestClaim = Instance.new('RemoteEvent')
	RequestClaim.Name = 'RequestClaim'
	RequestClaim.Parent = game:GetService('ReplicatedStorage')
	RequestClaim.OnServerEvent:Connect(function( Player : Player , ValueName : string )
		...
		local XPValue = EpisodePass:FindFirstChild( ( 'Episode' .. CurrentEpisode .. 'XP' ) ) :: NumberValue
