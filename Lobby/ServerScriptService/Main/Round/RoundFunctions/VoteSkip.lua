...
local function getRequiredDefeatsForSkip()
	return 0
end
...
local function debugSkip(message, source)
	if RunService:IsStudio() then
		warn(string.format(
			"[SkipDebug][Round %s/%s] %s | source=%s skip=%s open=%s votes=%s players=%s defeated=%s/%s required=%s",
			...
			tostring(getRequiredDefeatsForSkip())
			))
	end
end
