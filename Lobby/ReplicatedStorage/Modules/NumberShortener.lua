--[[
	...
	local Short = require(game.ReplicatedStorage.NumberShortener)
	-- This is how you shorten value (must be number)
	Short.en(Input)
--]]

-- put it in replicated storage so we can get the module both sevrerwise and localwise!



local Short = {}

Short.Time = function(timer)
	if timer > 0 then
		local newtimer = timer/60
		local secondsstring = 0
		local timestring = 0
		if string.find(newtimer, ".")  ~= nil then
			if string.len(tostring(timer - (math.floor(newtimer) * 60))) < 2 then
				secondsstring = "0"..timer - (math.floor(newtimer) * 60)
			else
				secondsstring = timer - (math.floor(newtimer) * 60)
			end
			if string.len(tostring(math.floor(newtimer))) < 2 then
				timestring = "0"..math.floor(newtimer)..":"..secondsstring
			else
				timestring = math.floor(newtimer)..":"..secondsstring
			end
		else
			timestring = newtimer..":".."00"
		end
		return(timestring)
	else
		return(0)
...
Short.en = function(Input)
	...
	elseif Input > 1.7976931348e321 then
		Input = "Infinite"		
	end
